...
def create_line_plot(datDF, years_range, selected_column,
...
def add_annotation(datDF, years_range, selected_column, data, annotate):
    ...
    # create the line fig
    line_fig = create_line_plot(datDF, years_range, selected_column)
    # filtering
    df = datDF[99:]
    df = df[(df['date'].dt.year >= years_range[0]) & (df['date'].dt.year <= years_range[1])]
    df.reset_index(inplace=True)
...
def add_annotation_3d(fig3d, data, click_3d, embedMtx, datDF, years_range, selected_column,
    ...
    # datDf: original datDf dataframe from the pickle file
    # years_range: selected years range from slider range
    # selected_column: selected column from dropdown
    # ball_x: the x position of 3d plot blue ball ( in case it previously exists )
    # ball_y: the y position of 3d plot blue ball ( in case it previously exists )
    ...
    if prev_ball:
        ...
        value = datDF[datDF['date'] == date][selected_column].values[0]
    ...
    # if clicked data from line plots then get the clicked point position from corresponding point number
        ...
        x = embedMtx[df.index[0]:df.index[-1], 0][selected_point['pointNumber']]
        y = embedMtx[df.index[0]:df.index[-1], 1][selected_point['pointNumber']]
        z = embedMtx[df.index[0]:df.index[-1], 2][selected_point['pointNumber']]
